		...
		<plugin>
			...
			<executions>
				...
				<execution>
					...
					<id>3-build-bonita-project</id> <3>
					<phase>package</phase>
					<goals>
						<goal>exec</goal>
					</goals>
...
<2> Download the bonita-la-builder for the target bonita runtime version
<3> Build the bonita project with the bonita-la-builder
<4> Tell maven to attach the zip artifact produced as the result of the build (so it can be used for maven publication in enterprise repositories)
