= How to build and deploy *"without stack management"* !

IMPORTANT: This page target teams deploying Bonita Living Applications on Bonita instances managed by themselves and not by `BCD controller` !
...
We vill explain how to easily build and deploy your Bonita Living Application repository without the use of the whole BCD tooling required for stack management. (ex. docker,ansible, terraform, python scripts, yaml scenario files, etc.)

"If you have no needs for the stack part, simply don't use it ! (aka. docker, mounted volumes, yaml scenario files, ansible playbooks and inventories, etc.)"
