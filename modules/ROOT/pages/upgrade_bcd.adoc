...
* Terraform metadata : "Terraform must store state about your managed infrastructure and configuration. This state is used by Terraform to map real world resources to your configuration, keep track of metadata, and to improve performance for large infrastructures." For more details, see the https://developer.hashicorp.com/terraform/language/state[Terraform documentation on "state"].
+
